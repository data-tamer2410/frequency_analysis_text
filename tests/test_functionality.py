...
def test_user_command_handler_search_word_cache():
    ...
    assert key == r"\b\w*player\w*\b False True False"
    ...
    assert key == r"\bplayer\b True False False"
    assert obj.search_cache[key]
    ...
    assert key == r"\bplayer\b False False False"
    assert obj.search_cache[key]
    ...
    assert key == r"\bгравець\b False False False"
    assert obj_uk.search_cache[key]

    key = obj_ru.search_cache_keys[0]
    assert key == r"\bигрок\b False False False"
    ...
    assert key == r"\bгравець\b True False False"
    assert obj_uk.search_cache[key]

    key = obj_ru.search_cache_keys[1]
    assert key == r"\bигрок\b True False False"
    assert obj_ru.search_cache[key]
...
def test_search_cache():
    ...
    assert obj2.search_cache_keys[0] == r"\bfootball\b False False False"
    for i in range(500):
    ...
    assert obj2.search_cache_keys[-1] == r"\bplayer\b False False False"
...
def test_load_file_from_pickle_json():
    ...
    assert (
        f_json.text,
        f_json.result_counter,
        f_json.datetime_created,
        f_json.language,
        ...
        obj.language,
        obj.search_cache_keys,
    )
    assert (
        f_pkl.text,
        f_pkl.result_counter,
        f_pkl.datetime_created,
        f_pkl.language,
        f_pkl.search_cache,
        f_pkl.search_cache_keys,
        f_pkl.history,
        f_pkl.redo_stack,
    ) == (
        obj.text,
